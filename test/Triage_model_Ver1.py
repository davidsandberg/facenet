...
#parameters = [{'kernel': ['rbf'], 'gamma': [1e-4, 1e-3, 1e-2, 1,1e2], 'C': np.logspace(-2, 1, 6)},  {'kernel': ['linear'], 'C': np.logspace(-2, 1, 4)}]
...
max_sample = 50
mega_im = PIL.Image.new('RGB', (182*10, 182*max_sample))
y_offset = 0
for x in range(0,max_sample):
    woo = tile_faces(test_with_names[x,1], test_with_names[x,2])
    mega_im.paste(woo, (0, y_offset))
    y_offset += 182
mega_im.show()

mega_im.save("First50_images_with_odds.jpeg", 'jpeg')
joblib.dump(svm_model, 'svm_model_large2.pkl')