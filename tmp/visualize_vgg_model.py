...
# Mean VGG-19 image
meanImage19 = np.array([103.939, 116.779, 123.68]).reshape(
    (1, 1, 1, 3)
...
def sqErrorLossContent(sess, modelGraph, layer):
    ...
    return (1 / (4 * N * M)) * tf.reduce_sum(
        tf.pow(modelGraph[layer] - sess.run(modelGraph[layer]), 2)
    )
...
def sqErrorLossStyle(sess, modelGraph):
    ...
    E = [
        intermediateCalc(sess.run(modelGraph[layerName]), modelGraph[layerName])
        for layerName, _ in styleLayers
    ]
    W = [w for _, w in styleLayers]
    return sum(
        [W[layerNumber] * E[layerNumber] for layerNumber in range(len(styleLayers))]
    )
...
imageNoise = np.random.uniform(-20, 20, (1, outputHeight, outputWidth, 3)).astype(
    "float32"
)
