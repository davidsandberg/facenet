...
tf.app.flags.DEFINE_boolean(
    "use_fp16", False, "Use half floats instead of full floats if True."
)
...
def fake_data(num_images):
    ...
    data = np.ndarray(
        shape=(num_images, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS), dtype=np.float32
    )
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    fc1p_weights = tf.Variable(  # fully connected, depth 512.
        tf.truncated_normal([512, 2], stddev=0.1, seed=SEED, dtype=data_type())
    )
    fc1p_biases = tf.Variable(tf.constant(0.1, shape=[2], dtype=data_type()))
    fc2_weights = tf.Variable(
        tf.truncated_normal([2, NUM_LABELS], stddev=0.1, seed=SEED, dtype=data_type())
    )
    ...
    def calculate_embeddings(data, sess):
        ...
        for begin in xrange(0, size, EVAL_BATCH_SIZE):
            ...
            if end <= size:
                predictions[begin:end, :] = sess.run(
                    eval_embeddings, feed_dict={eval_data: data[begin:end, ...]}
                )
            else:
                batch_predictions = sess.run(
                    eval_embeddings, feed_dict={eval_data: data[-EVAL_BATCH_SIZE:, ...]}
                )
                predictions[begin:, :] = batch_predictions[begin - size :, :]
        return predictions
    ...
    with tf.Session() as sess:
        ...
        for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
            ...
            if step % EVAL_FREQUENCY == 0:
                ...
                print(
                    "Minibatch loss: %.3f  %.3f, learning rate: %.6f"
                    % (l, cl * beta, lr)
                )
        ...
        for n in range(0, 10):
            ...
            plt.plot(
                train_embeddings[idx, 0], train_embeddings[idx, 1], color_list[n] + "."
            )
        plt.show()
