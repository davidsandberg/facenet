...
def inference(images, pool_type, use_lrn, keep_probability, phase_train=True):
  ...
  incept4e = facenet.inception(incept4a, 640, 2, 0, 160, 256, 64, 128, 3, 0, 2, 'MAX', 'incept4e', phase_train=phase_train, use_batch_norm=True)
  
  incept5a = facenet.inception(incept4e,    1024, 1, 256, 96,  384, 0, 0, 3, 96,  1, pool_type, 'incept5a', phase_train=phase_train, use_batch_norm=True)
  incept5b = facenet.inception(incept5a, 736, 1, 256, 96, 384, 0, 0, 3, 96, 1, 'MAX', 'incept5b', phase_train=phase_train, use_batch_norm=True)
  pool6 = facenet.apool(incept5b,  3, 3, 1, 1, 'VALID')

  resh1 = tf.reshape(pool6, [-1, 736])
  affn1 = facenet.affine(resh1, 736, 128)
