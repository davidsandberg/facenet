...
def main(args):
    ...
    vggface16.load('../data/vgg_face.mat', t_preprocessed)
    ...
    def render_naive(t_obj, img0=img_noise, iter_n=20, step=1.0, feature=-1):
        ...
        for j in range(iter_n):
            ...
            img += g*step
            print('iter=%03d  score=%6.2f' % (j, score))
            if j % 100 == 0 and feature>=0:
                plt.imsave('feat%03d_iter%03d' % (feature, j), visstd(img))
                #img_list.append(visstd(img))
        return img
    ...
    params = [op.name for op in tf.get_default_graph().get_operations() if op.type=='Const']
    param_dims = {param: tf.get_default_graph().get_tensor_by_name('%s:0' % param).get_shape() for param in params}

#     tensor = tf.get_default_graph().get_tensor_by_name('Const:0')
#     t = sess.run(tensor)
#     img = np.zeros((3+2)*8, (3+2)*8, 3)
#     for i in range(8):
#         for j in range(8):
#             img[]
    ...
    for i in range(16):
        ...
        #plt.subplot(4,4,i+1)
        img = render_naive(T(layer)[:,:,:,channels[i]], iter_n=1000, feature=i)
